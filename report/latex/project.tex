...
\begin{abstract}
% TODO: abstract, one paragraph, max 150 words.

\textbf{Keywords:}
% TODO: keywords separated by semicolons
\end{abstract}
...
% TODO: placeholder — verbatim quote from source article; paraphrase and cite before submission.
...
% TODO: one-paragraph preview of the main findings (replaces the former "Main Findings" subsection).
...
This results in a final dataset of 123,879 reader$\times$word data points: from 142,125 raw data points (75 readers $\times$ merged reading measures), 140,325 remain after dropping sentence-edge words, 126,382 after dropping skips (TFT = 0), and 123,879 after per-reader 3$\cdot$SD outlier removal. The final breakdown by text domain and reader expertise is given in Table~\ref{tab:breakdown}.
...
The initial scraping results consisted of approximately 6,000 articles from both domains. We then observed the pool of documents, deleted duplicates, and removed articles outside of the desired domain. In addition, math symbols and references were stripped from the text. Corpus statistics are given in Table~\ref{tab:corpora}.
...
such that each model is evaluated on both in-domain and out-of-domain data (Figure~\ref{fig:perplexity}).
...
\subsection{Prompt-Based Domain Priors}
...
The suprisal of a word is then calculated as the sum of its subtokens' surprisal (Figure~\ref{fig:sentence-overlay}).

\begin{figure*}[t]
  \begin{center}
    \includegraphics[width=0.9\textwidth]{../../figures/fig2_sentence_overlay_german-gpt2_b0.png}
  \end{center}
...
Figure~\ref{fig:mean-surprisal} shows mean surprisal over training steps, and Figure~\ref{fig:delta-ll} shows $\Delta$LL per arm; full model comparisons are given in Tables~\ref{tab:gpt-results} and~\ref{tab:llama-results}.
...
% TODO: AI-use declaration only in the initial submission (per CogSci template); full
% acknowledgments (contributions, funding) in the final submission.
