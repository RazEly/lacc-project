"""Download and extract PoTeC eye-tracking data from the OSF repository.

Adapted from the upstream PoTeC `download_data_files.py`: the only change is
that ``output_folder`` may be an absolute path, so callers can place the data
inside the cloned ``data/potec`` checkout instead of next to this file.
"""
import argparse
import os
...
def download_data(extract: bool, output_folder: str, download_asc: bool, download_fixations: bool,
    ...
    folder = out if out.is_absolute() else Path(__file__).parent / out
