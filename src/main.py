...
MEASURE = "TFT"
# Surprisal arms swept against the shared no-surprisal baseline.
MODEL_ARMS = ("baseline", "prompted", "prompt_neutral", "aligned")
# every checkpoint: indices 1..N pair to config.DAPT_CHECKPOINT_STEPS (0 = baseline).
CHECKPOINT_INDICES = list(range(1, len(config.DAPT_CHECKPOINT_STEPS) + 1))
...
def compute_model_surprisal(slug: str, name: str, words, priors: dict) -> dict:
    ...
    # a shared context budget, scored on EVERY word. The physics/biology domain
    # priors build the reader-aligned ``prompted`` arm (discipline-matched per
    # reader); the neutral prior is the single fixed-domain pseudo-test. Each arg is
...
def main() -> None:
    ...
    if bundles is not None:
        ...
        print(
            f"  priors: {config.N_PRIOR_PASSAGES}× physics/biology (aligned arm) + "
            f"neutral (pseudo-test) from each Wikipedia corpus, averaged per word "
    ...
    if manifests:
        ...
        # PoTeC-stimuli lines (SHOW_STIMULI): per-checkpoint token perplexity on
        # the physics/biology stimulus texts, derived from the cached per-word
        # surprisal — only the tokenizer is loaded, no extra forward passes.
        stimuli = None
        if SHOW_STIMULI:
            stimuli = pd.concat(
                [
    ...
    viz.rt_vs_surprisal(rm, baseline_surp, measure=MEASURE, slug=fig_lm["slug"])
