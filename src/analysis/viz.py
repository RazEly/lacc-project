...
def sentence_surprisal_example(
    ...
    ckpts = surp_versions[(surp_versions["domain"] == domain) & (surp_versions["index"] > 0)]
    for (_, step), g in sorted(ckpts.groupby(["index", "epoch"])):
        axes[1].plot(x, _surp(g, "surprisal") - base, marker="o", label=f"step {int(step)}")
    ...
    for cond, color in PROMPT_COLORS.items():
        ...
        if col in prompt_surp.columns:
            axes[2].plot(x, _surp(prompt_surp, col) - base, color=color,
...
def mean_surprisal_over_steps(
    ...
    aligned = (
        ...
        .groupby("epoch")["surprisal"].mean().reset_index()
    )

    p = prompt_surp.merge(dom, on=WORD_KEY)
    aligned_prompt = float(
        np.where(p["text_domain"] == "physics", p["s_prompt_physics"], p["s_prompt_biology"]).mean()
    )

    plt = _plt()
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.plot(aligned["epoch"].clip(lower=1), aligned["surprisal"],
            color="tab:blue", marker="o", label="aligned model")
    ax.axhline(aligned_prompt, color="tab:orange", ls="--", label="aligned prompt")
    if "s_prompt_neutral" in p.columns:
        ax.axhline(p["s_prompt_neutral"].mean(), color="gray", ls="--", label="neutral prompt")
    ax.set_xscale("log")
...
def delta_ll_curves(results: pd.DataFrame, out_dir: Path = FIGURES_DIR) -> pd.DataFrame:
    ...
    fig, axes = plt.subplots(1, len(slugs), figsize=(5.5 * len(slugs), 4),
    ...
    for ax, slug in zip(axes[0], slugs):
        ...
        for model, g in sub.groupby("model"):
            if g["epoch"].isna().all():  # checkpoint-independent: intercept only
                ...
                ax.plot(g["epoch"].clip(lower=1), g["delta_ll"], marker="o", label=model)
        ax.set_xscale("log")
...
def rt_vs_surprisal(
    ...
    """
    df = rm.merge(baseline_surp[WORD_KEY + ["surprisal"]].drop_duplicates(WORD_KEY), on=WORD_KEY)
    df = df[df[measure] > 0].assign(log_rt=lambda x: np.log(x[measure]))

    binned = (
        df.groupby(pd.qcut(df["surprisal"], n_bins, duplicates="drop"), observed=True)
    ...
    ax.plot(binned["surprisal"], binned["log_rt"], color="tab:red", marker="o",
